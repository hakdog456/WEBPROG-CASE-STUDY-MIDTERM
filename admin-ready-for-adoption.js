	...
	const fetchTransaction = async () => {
		try {
			...
			if (transactionId) {
				...
				const response = await fetch('ensure-transaction-by-status.php?status=' + encodeURIComponent('Adopted'));
				data = await response.json();
			}
			...
			// Populate payment details from transaction
			if (currentTransaction.dateTimeCreated && dateValue) {
				const date = new Date(currentTransaction.dateTimeCreated);
